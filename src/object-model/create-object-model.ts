...
const getPatches = (
  ...
  const names = getPnames(wad, nameToIndexMap, processed).map((p) =>
    p.toUpperCase()
  );
...
const getTextures = (
  ...
  const t1 = <Texture[]>(
    getLump(wad, nameToIndexMap, processed, "TEXTURE1", "texture")
  );
  const t2 = <Texture[]>(
    getLump(wad, nameToIndexMap, processed, "TEXTURE2", "texture")
  );
...
export const createObjectModel = (
  ...
  if (options.demos) objectModel.demos = getDemos(wad, indexMap, processed);
  ...
  if (options.flats) objectModel.flats = getFlats(wad, indexMap, processed);

  if (options.genmidi)
    objectModel.genmidi = getLump(wad, indexMap, processed, "GENMIDI", "raw");

  if (options.levels) objectModel.levels = getLevels(wad, processed);

  if (options.music) objectModel.music = getMusic(wad, indexMap, processed);
  ...
  if (options.sounds) objectModel.sounds = getSounds(wad, indexMap, processed);
  ...
  if (options.ui) objectModel.ui = getUi(wad, indexMap, processed);
